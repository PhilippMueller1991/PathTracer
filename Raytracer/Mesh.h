...
public:
	...
	static bool isOnSurface(const Vec3f& pos, const Vec3f& v0, const Vec3f& v1, const Vec3f& v2)
	{
		...
		// Test simple plane equation
		Vec3f n0 = getNormal(v0, v1, v2);
		Vec3f n1 = getNormal(pos, v1, v2);
		Vec3f diff = n0 - n1;
		if (fabs(diff.x) > EPS || fabs(diff.y) > EPS || fabs(diff.z) > EPS)
			return false;
...
private:
	...
	std::vector<Vec2f> uvs;	// Not used right now, but filled
